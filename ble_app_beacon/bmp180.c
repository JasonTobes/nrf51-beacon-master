...
void twi_handler(nrf_drv_twi_evt_t const * p_event, void * p_context)
{
  ...
  switch(p_event->type)
      ...
      if (p_event->xfer_desc.type == NRF_DRV_TWI_XFER_TX)
	{
	  ...
	  if (ctx.return_data) {
	    err_code = nrf_drv_twi_rx(&ctx.m_twi_sensor, BMP180_ADDR, (uint8_t*)ctx.return_data, ctx.return_data_len);
	    ctx.return_data = 0;
...
char bmp180_readBytes(unsigned char *values, char length)
{
  ...
  _error = nrf_drv_twi_tx(&ctx.m_twi_sensor, BMP180_ADDR, values, 1, false);
  while (!ctx.m_xfer_done);
#else
  _error = nrf_drv_twi_tx(&ctx.m_twi_sensor, BMP180_ADDR, values, 1, false);
  _error = nrf_drv_twi_rx(&ctx.m_twi_sensor, BMP180_ADDR, values, length);
...
{
  ret_code_t _error = nrf_drv_twi_tx(&ctx.m_twi_sensor, BMP180_ADDR, values, length, false);
...

{
  ...
  //ctx.m_twi_sensor = m_twi_sensor; //NRF_DRV_TWI_INSTANCE(0);

  ret_code_t err_code;
  err_code = nrf_drv_twi_init(&ctx.m_twi_sensor, &twi_bmp180_config, twi_handler, &ctx);

  APP_ERROR_CHECK(err_code);
  nrf_drv_twi_enable(&ctx.m_twi_sensor);
}
