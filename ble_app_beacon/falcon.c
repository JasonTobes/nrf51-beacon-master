...
/**
    Parses data into Falcon data format scale
    @param *data pointer to falcon_sensor_t object
...
void parseSensorData(struct falcon_sensor_t* data, int32_t raw_t, uint32_t raw_p, uint32_t raw_h, uint16_t vbat, int32_t acc[3])
{
...
/**
    Parses sensor values into FalconTag format.
    @param char* data_buffer character array with length of 14 bytes
*/
void encodeToSensorDataFormat(uint8_t* data_buffer, struct falcon_sensor_t* data)
{
...
void encodeToUrlDataFromat(char* buf, falcon_sensor_t* data)
{
